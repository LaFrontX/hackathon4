...
if result.get("offered_subscription"):
                    driver.offered_subscription = result["offered_subscription"]
                    db.commit()
                return result
...
@app.get("/mobile/prediction/{driver_id}")
async def mobile_prediction(driver_id: int, db: Session = Depends(get_db)):
    driver = db.query(Driver).filter(Driver.id
 == driver_id).first()
    if not driver:
        raise HTTPException(status_code=404, detail="Водитель не найден")
...
(
            ...
            if response.status_code == 200:
                ...
                return {
                    ...
                    "offered_subscription": result.get("offered_subscription"),
...
@app.get("/mobile/subscription/{driver_id}")
def mobile_subscription(driver_id: int, db: Session = Depends(get_db)):
    driver = db.query(Driver).filter(Driver.id
 == driver_id).first()
    if not driver:
        raise HTTPException(status_code=404, detail="Водитель не найден")
    
    trips = driver.trips_per_month
    ...
    if subscription:
        driver.offered_subscription = subscription
        db.commit()
    
    return {
        "recommended_subscription": subscription,
